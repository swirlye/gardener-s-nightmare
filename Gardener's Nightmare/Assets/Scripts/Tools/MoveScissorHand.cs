...
public class MoveScissorHand : MonoBehaviour
{
    ...
    private async Task UpdateHandlePosition(InputDeviceCharacteristics hand)
    {
        ...
        if (gripState)
        {
            ...
            while (_grabbing)
            {
                ...
                Vector3 handPosition = new Vector3(_grabbingHand.position.x, _pivot.position.y, _grabbingHand.position.z);
                Vector3 grabberPosition = new Vector3(_grabber.position.x, _pivot.position.y, _grabber.position.z);

                float angleTowards = SignedAngleBetween(handPosition - _pivot.position, grabberPosition - _pivot.position, _pivot.up, false);
                ...
                transform.RotateAround(_pivot.position, Vector3.up, angleTowards);
